...
def function_exceptions(function):
    try:
        v = function()
        if v != None:
            return v
        ...
        print("******************")
        print(function.__name__, "오류 : ", end=" ")
        if DEBUG:
...
secrets = function_exceptions(read_json)
...
function_exceptions(remove_popup)
    ...
    for index, li in enumerate(li_list):
        ...
        for file in ul_list[0].find_elements_by_tag_name('li'):
            ...
            if ((lecture_list[now_index] in my_text) and ("동영상" in my_text)):
                (from_d, until_d) = function_exceptions(calculate_time)

                # 출석인정기간인 경우
                if (from_d <= time_now) and (time_now <= until_d):
                    function_exceptions(watching_lecture)
